...
```{r}
data.frame(x=1:20) %>% mutate(y=displex_exp_law(x)) %>% ggplot(aes(x=x,y=y)) + geom_line()
```
y se integra las distintas valoraciones mediante una ley de suma probabilística:

```{r}
expYprob <- displex_availability(d)
expYprob %>% arrange(-availability) %>% ggplot(aes(x=seq_along(words),y=availability)) + geom_line()
```
Si seleccionamos los elementos más representativos y los que menos:

```{r}
expYprob %>% arrange(-availability) %>% head(10) %>% select(words, availability)
```
...
lopezstrass %>% arrange(-availability) %>% select(words,availability)  %>% head(10) %>% select(words,availability)
```
```{r}
lopezstrass %>% arrange(-availability) %>% ggplot(aes(x=seq_along(words),y=availability)) + geom_line()
```
...
lopezstrass_nm <- displex_availability(d,law=lopez_law,reduce=lopez_reduce)
...
```{r}
inner_join(lopezstrass %>% select(words, availability), lopezstrass_nm %>% select(words, availability), by="words") %>% mutate(diff=availability.x - availability.y) %>% select(diff) %>% summarise(maxdiff=max(diff))
```
...
```{r}
analysisComp <-
inner_join(expYprob %>% arrange(-availability) %>% mutate(eOrd=seq_along(words)) %>% rename(eAvailability=availability),
           lopezstrass %>% arrange(-availability) %>% mutate(lOrd=seq_along(words)) %>% rename(lAvailability=availability),
           by="words") %>%
...
```{r}
analysisComp %>% ggplot(aes(x=eAvailability,y=lAvailability)) + geom_point(aes(color=diffOrd))
```
...
```{r}
data.frame(x=1:20) %>% mutate(y=displex_zipf_law(x)) %>% ggplot(aes(x=x,y=y)) + geom_line()
```
...
zipfYexp %>% arrange(-availability) %>% ggplot(aes(x=seq_along(words),y=availability)) + geom_line()
```
Es posible observar que hay muchas valoraciones que son indistinguibles de uno. 
```{r}
zipfYexp %>% filter(centers=="01") %>% arrange(-availability) %>% head(10) %>% select(words, availability)
```
...
```{r}
analysisComp <-
inner_join(zipfYexp %>% arrange(-availability) %>% mutate(eOrd=seq_along(words)) %>% rename(eAvailability=availability),
           lopezstrass %>% arrange(-availability) %>% mutate(lOrd=seq_along(words)) %>% rename(lAvailability=availability),
           by="words") %>%
  mutate(diffOrd=eOrd - lOrd)
analysisComp %>% ggplot(aes(x=eOrd,y=lOrd)) + geom_point(aes(color=diffOrd))
...
```{r}
zipfYmean <- displex_availability(d,law=displex_zipf_law,reduce=mean)
zipfYmean %>% arrange(-availability) %>% ggplot(aes(x=seq_along(words),y=availability)) + geom_line()
```

De nuevo se observa una relación poco clara con la medida de López-Strassburger:
```{r}
analysisComp <-
inner_join(zipfYmean %>% arrange(-availability) %>% mutate(eOrd=seq_along(words)) %>% rename(eAvailability=availability),
           lopezstrass %>% arrange(-availability) %>% mutate(lOrd=seq_along(words)) %>% rename(lAvailability=availability),
           by="words") %>%
  mutate(diffOrd=eOrd - lOrd)
analysisComp %>% ggplot(aes(x=eOrd,y=lOrd)) + geom_point(aes(color=diffOrd))
...
```{r}
data.frame(x=1:20) %>% mutate(y=displex_zipf_law(x, d=1)) %>% ggplot(aes(x=x,y=y)) + geom_line()
```
Obteniéndose una forma más conocida

```{r}
avSanc <- displex_availability(d,law=function(x) {displex_zipf_law(x,d=1)},reduce=displex_additive_law)
avSanc %>% arrange(-availability) %>% ggplot(aes(x=seq_along(words),y=availability)) + geom_line()
```
...
```{r}
analysisComp <-
inner_join(avSanc %>% arrange(-availability) %>% mutate(eOrd=seq_along(words)) %>% rename(eAvailability=availability),
           lopezstrass %>% arrange(-availability) %>% mutate(lOrd=seq_along(words)) %>% rename(lAvailability=availability),
           by="words") %>%
  mutate(diffOrd=eOrd - lOrd)
analysisComp %>% ggplot(aes(x=eAvailability,y=lAvailability)) + geom_point(aes(color=diffOrd))
...
data.frame(levels  = levels, measures = gs) %>% 
  ...
  geom_line(aes(x=seq_along(levels), y=gs),colour="dark gray") + xlab("") + ylab("nivel de corte (valor de alfa)") + 
  geom_text(aes(x=length(levels), y=1), label="Tamaño del alfa-corte",hjust=1,vjust=0) +
  geom_text(aes(x=length(levels), y=0), label="Valor de alfa",hjust=1,vjust=1)
```
